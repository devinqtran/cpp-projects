...
bool ServerGame::place_ship(int player_id, int row, int col, char orientation)
{
    ...
    if (player.ships_placed >= MAX_SHIPS)
        return false;
    ...
    for (int i = 0; i < length; ++i)
    {
        ...
        if (player.board[r][c] != CellState::EMPTY)
            return false;
    }
...
{
    // Loop through both players (Player 0 and Player 1)
    for (int p = 0; p < 2; ++p)
    {
        // Reset their stats
        players_[p].ships_placed = 0;
        players_[p].ship_cells_remaining = 0;

        // Wipe their board completely clean
        for (int r = 0; r < BOARD_SIZE; ++r)
        {
            for (int c = 0; c < BOARD_SIZE; ++c)
            {
                players_[p].board[r][c] = CellState::EMPTY;
            }
        }
    }
}