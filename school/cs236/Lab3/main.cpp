...
int main(int argc, char** argv) {
    ...
    while (currentToken.getType() != END) { // Adjust if your Token class uses a different getter
        tokens.push_back(currentToken);
        currentToken = scanner.scanToken();
    }
    tokens.push_back(currentToken); // Add the END token
    ...
    DatalogProgram program = parser.getProgram(); // Assuming you have this getter in Parser
    Interpreter interpreter(program);
    interpreter.evaluateSchemes();

    // 6. Print the database to verify
